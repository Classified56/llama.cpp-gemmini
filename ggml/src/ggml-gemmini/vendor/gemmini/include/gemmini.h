...
//  printf("tiled_matmul_outer: config_ld D\n");
  ...
  if (act == IGELU) {
    ...
    printf("tiled_matmul_outer: igelu config_norm\n");
  ...
  if (act == SOFTMAX) {
    ...
    printf("tiled_matmul_outer: softmax config_norm\n");
    printf("config_norm args: q_const=%d q_scale=%d qb=%d qc=%d\n",
       qln2, qln2_inv, qb, qc);
    gemmini_config_norm(qln2, 0, 0, 1, 0, qb, qc);
    printf("tiled_matmul_outer: softmax config_norm2\n");
...
//  printf("tiled_matmul_outer: fenced waiting for function end\n");
  gemmini_fence();
}
...
static void tiled_matmul(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  if (tiled_matmul_type == OS || tiled_matmul_type == WS) {
    // printf("tiled_matmul_outer start\n");
